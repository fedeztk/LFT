...
public class Translator {
    ...
    public void bexpr(int l_false) {
        if (look.tag == '(') { // bexpr>>> (bexprp)
            match('(');
            bexprp(l_false);
            match(')');
        } else {
    ...
    public void bexprp(int l_false) {
        if (look.tag == Tag.RELOP) {
            switch (((Word) look).lexeme) {
                case "==": // == (=)
                    ...
                    expr();
                    expr();
                    code.emit(OpCode.if_icmpne, l_false); // == >>> <>
                    /*
                     * code.emit(OpCode.if_icmpeq, l_true); code.emit(OpCode.GOto, l_false);
                     */
                    break;
                case ">": // >
                    match(Tag.RELOP);
                    expr();
                    expr();
                    code.emit(OpCode.if_icmple, l_false); // > >>> <=
                    /*
                     * code.emit(OpCode.if_icmpgt, l_true); code.emit(OpCode.GOto, l_false);
                     */
                    break;
                case "<": // <
                    match(Tag.RELOP);
                    expr();
                    expr();
                    code.emit(OpCode.if_icmpge, l_false); // < >>> >=
                    /*
                     * code.emit(OpCode.if_icmplt, l_true); code.emit(OpCode.GOto, l_false);
                     */
                    break;
                case ">=": // >=
                    match(Tag.RELOP);
                    expr();
                    expr();
                    code.emit(OpCode.if_icmplt, l_false); // >= >>> <
                    /*
                     * code.emit(OpCode.if_icmpge, l_true); code.emit(OpCode.GOto, l_false);
                     */
                    break;
                case "<=": // <=
                    match(Tag.RELOP);
                    expr();
                    expr();
                    code.emit(OpCode.if_icmpgt, l_false); // <= >>> >
                    /*
                     * code.emit(OpCode.if_icmple, l_true); code.emit(OpCode.GOto, l_false);
                     */
                    break;
                case "<>": // <> (!=)
                    match(Tag.RELOP);
                    expr();
                    expr();
                    code.emit(OpCode.if_icmpeq, l_false);
                    /*
                     * code.emit(OpCode.if_icmpne, l_true); code.emit(OpCode.GOto, l_false);
                     */
                    break;
                default:
